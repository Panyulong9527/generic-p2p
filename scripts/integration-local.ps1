...
$runRoot = Join-Path $projectRoot (".integration-run-" + $runId)
...
try {
    ...
    $seedProcess = Start-Process -FilePath $binPath `
        -ArgumentList @("serve", "--path", $sourcePath, "--listen", "127.0.0.1:$SeedPort", "--data-dir", $manifestDir, "--lan", "--lan-addr", "127.0.0.1:$LanPort") `
    ...
    $leecher2Process = Start-Process -FilePath $binPath `
        -ArgumentList @("get", "--manifest", $manifestPath, "--store-dir", $store2Dir, "--out", $out2Path, "--listen", "127.0.0.1:$LeecherPort", "--lan", "--lan-addr", "127.0.0.1:$LanPort", "--peer", "127.0.0.1:$SeedPort") `
        -WorkingDirectory $projectRoot `
    ...
    Write-Host "Starting leecher3 with LAN auto-discovery"
    $leecher3Process = Start-Process -FilePath $binPath `
        -ArgumentList @("get", "--manifest", $manifestPath, "--store-dir", $store3Dir, "--out", $out3Path, "--lan", "--lan-addr", "127.0.0.1:$LanPort") `
        -WorkingDirectory $projectRoot `
    ...
    Select-String -Path $leecher3OutLog -Pattern "lan_peer_discovered|peer_have_received|piece_downloaded|content_assembled" |
    ...
    foreach ($proc in @($leecher3Process, $leecher2Process, $seedProcess)) {
